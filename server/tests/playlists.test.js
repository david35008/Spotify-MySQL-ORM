...
describe('check playlists routs', () => {
  ...
  it('Can create playlist', async () => {
    const { body } = await request(server).post('/playlists').send(playlistMock);
    expect(body.name).toBe(playlistMock.name)
  })

  it('Can get single playlist', async () => {
    const { body: newPlaylist } = await request(server).post('/playlists').send(playlistMock);
    const { body: getSinglePlaylistResponseBody } = await request(server).get(`/playlists/byId/${newPlaylist.id}`);
